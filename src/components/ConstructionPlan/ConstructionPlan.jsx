...
import AllocationTimelineModal, { getCurrentYear } from './AllocationTimelineModal'; // <-- SỬA LẠI DÒNG NÀY
    ...
    return (
        <Grid item xs={12} sm={6} md={4}>
            <motion.div variants={itemVariants} whileHover={{ y: -5 }} transition={{ type: "spring", stiffness: 300 }}>
                <Card sx={{ borderRadius: 4, boxShadow: "0 4px 12px 0 rgba(0,0,0,0.05)", border: `1px solid ${theme.palette.divider}`, height: "100%" }}>
                    <CardContent sx={{ display: "flex", alignItems: "center", p: 3, gap: 2 }}>
                        <Avatar sx={{ width: 64, height: 64, color: "#fff", background: gradient, boxShadow: `0px 6px 12px ${alpha(theme.palette[color].main, 0.3)}` }}>
...
export default function ConstructionPlan() {
            ...
            try {
                ...
                const projectsWithTotals = await Promise.all(
                    projectsData.map(async (project) => {
                        ...
                        const totalHSKH = planningSnapshot.docs.reduce((sum, doc) => sum + (Number(doc.data().amount) || 0), 0);
    ...
    const handleCreateProject = useCallback(async () => {
        ...
        } catch (e) { /* Lỗi đã được toast.promise xử lý */ }
    }, [newProject]);
    ...
    const columns = useMemo(
        () => [
            { field: "name", headerName: "Tên Công Trình", flex: 1, minWidth: 350, renderCell: (params) => (<Typography variant="body2" fontWeight={600}>{params.value}</Typography>), },
            { field: "totalAmount", headerName: "Doanh Thu Dự Kiến", width: 200, type: "number", align: "right", headerAlign: "right", renderCell: (params) => (<Typography variant="body2" sx={{ fontFamily: "Roboto Mono, monospace" }}>{formatNumber(params.value)} ₫</Typography>), },
            { field: "revenueHSKH", headerName: "Doanh thu HSKH", width: 200, type: "number", align: "right", headerAlign: "right", renderCell: (params) => (<Typography variant="body2" fontWeight={600} sx={{ fontFamily: "Roboto Mono, monospace", color: 'primary.main' }}>{formatNumber(params.value)} ₫</Typography>), },
            { field: "type", headerName: "Loại", width: 120, renderCell: (params) => { const color = chipColorByType[params.value] || "default"; return (<Chip label={params.value} size="small" sx={{ fontWeight: 600, color: theme.palette[color].dark, backgroundColor: alpha(theme.palette[color].main, 0.15), borderRadius: "6px", }} />); }, },
            { // <-- CỘT PHÂN BỔ ĐÃ ĐƯỢC THAY THẾ HOÀN TOÀN
                ...
                sortable: false,
                renderCell: (params) => {
                    const isCurrentlyAllocated = isProjectCurrentlyAllocated(params.row);
                    return (
                        <Tooltip title="Click để xem & sửa lịch phân bổ">
                            <Chip
                                icon={isCurrentlyAllocated ? <CheckCircleOutline /> : <HighlightOff />}
                                label={isCurrentlyAllocated ? "Đang Phân Bổ" : "Tạm Ngưng"}
                                size="medium"
                                onClick={(e) => {
                                    e.stopPropagation();
                                ...
                                sx={{
                                    ...
                                    borderRadius: "8px",
                                    px: 1,
                                    color: isCurrentlyAllocated ? 'success.dark' : 'error.dark',
            ...
            { field: "actions", headerName: "Thao Tác", width: 100, align: "right", headerAlign: "right", sortable: false, renderCell: (params) => (<ProjectActionsMenu onEdit={() => handleOpenEditDialog(params.row)} onDelete={() => handleOpenDeleteDialog(params.row)} />), },
        ],
    ...
    return (
        <Box sx={{ p: { xs: 2, md: 3, lg: 4 }, bgcolor: "#F8F9FC", minHeight: "100vh" }}>
                ...
                <motion.div variants={itemVariants}>
                    <Paper elevation={0} sx={{ mt: 3, borderRadius: 5, overflow: "hidden", border: `1px solid ${theme.palette.divider}` }}>
                        <Stack direction={{ xs: "column", md: "row" }} spacing={2} p={3} justifyContent="space-between" alignItems="center" bgcolor="background.paper">
                            <TextField variant="filled" hiddenLabel placeholder="Tìm kiếm công trình..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} InputProps={{ startAdornment: (<InputAdornment position="start"><Search color="action" /></InputAdornment>), disableUnderline: true, }} sx={{ width: { xs: "100%", md: 400 }, "& .MuiFilledInput-root": { borderRadius: "12px", bgcolor: "grey.100", }, }} />
                            <Button variant="contained" size="large" startIcon={<AddCircleOutline />} onClick={() => setOpenAddDrawer(true)} sx={{ borderRadius: "12px", width: { xs: "100%", md: "auto" }, boxShadow: "none", "&:hover": { boxShadow: `0 4px 12px ${alpha(theme.palette.primary.main, 0.3)}` }, }}>
                                Thêm Công Trình
                            </Button>
                        </Stack>
                        ...
                        <Box sx={{ height: 650, width: "100%" }}>
                            <DataGrid
                                ...
                                sx={{
                                    border: "none",
                                    "& .MuiDataGrid-columnHeaders": { borderTop: `1px solid ${theme.palette.divider}`, borderBottom: `1px solid ${theme.palette.divider}`, },
                                    "& .MuiDataGrid-columnHeaderTitle": { fontWeight: "700", color: "text.secondary", textTransform: "uppercase", fontSize: "0.8rem", },
                                    "& .MuiDataGrid-row": { cursor: "pointer", "&:hover": { backgroundColor: alpha(theme.palette.primary.main, 0.04), }, },
                                    "& .MuiDataGrid-cell": { borderBottom: `1px solid ${theme.palette.divider}`, alignItems: "center", display: "flex", },
                                    "& .MuiDataGrid-cell:focus-within, & .MuiDataGrid-columnHeader:focus-within": { outline: "none !important" },
            ...
            <ProjectFormDrawer open={openAddDrawer} onClose={() => setOpenAddDrawer(false)} project={newProject} setProject={setNewProject} onSave={handleCreateProject} isEdit={false} />
            
            {projectToEdit && (<ProjectFormDrawer open={openEditDrawer} onClose={() => setOpenEditDrawer(false)} project={projectToEdit} setProject={setProjectToEdit} onSave={handleUpdateProject} isEdit={true} />)}
            ...
            <Dialog open={!!projectToDelete} onClose={() => setProjectToDelete(null)} PaperProps={{ sx: { borderRadius: 4 } }}>
                ...
                <DialogContent><Typography>Bạn có chắc chắn muốn xoá công trình "{projectToDelete?.name}"? Hành động này không thể hoàn tác.</Typography></DialogContent>
