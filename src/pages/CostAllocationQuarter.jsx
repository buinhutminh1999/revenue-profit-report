...
import React, {
  ...
  export default function CostAllocationQuarter() {
    ...
    // 2) load extraRows (recompute carryOver from prev quarter)
    useEffect(() => {
      (async () => {
        ...
        if (currSnap.exists() && Array.isArray(currSnap.data().mainRows)) {
          const rows = currSnap.data().mainRows.map(r=>{
            const m     = mainRows.find(x=>x.id===r.id);
            const qv    = toNum(m?.monthly?.T1)+toNum(m?.monthly?.T2)+toNum(m?.monthly?.T3);
            const alloc = Math.round(qv * toNum(m?.percentThiCong)/100);
            let used=0;
            projects.forEach(p=>{
              ...
              const dc  = toNum(
                projData[p.id]?.directCost ??
                projData[p.id]?.items?.find(it=>
                  (it.description||"").trim().toLowerCase()===r.label.trim().toLowerCase()
                )?.directCost
              );
              used += Math.round((rev * toNum(r.pct))/100 - dc);
            });
            return { ...r, carryOver: toNum(r.carryOver) + alloc - used };
          });
          setExtraRows(rows);
        } else {
    ...
    },[year,quarter,projects,projData,mainRows]);
    ...
    useEffect(()=>{
      ...
      (async()=>{
        ...
        for(const p of projects){
          const snap = await getDoc(doc(db,"projects",p.id,"years",String(year),"quarters",quarter));
    ...
    const getDC = useCallback((pId,rowId)=>{
      ...
      if(raw===undefined){
        ...
        raw = projData[pId]?.items?.find(it=>
    ...
    const rows = useMemo(()=>{
      const head = cats.map(cat=>{
        ...
        projects.forEach(p=>{
          r[p.id] = cat.key==="overallRevenue"? toNum(projData[p.id]?.overallRevenue):0;
          r[`${p.id}_dc`] = "";
        });
      ...
      const extra = extraRows.map(ex=>{
        const r={id:ex.id,label:ex.label,pct:ex.pct,carryOver:toNum(ex.carryOver)};
        projects.forEach(p=>{
          const rev = toNum(projData[p.id]?.overallRevenue);
        ...
        r.used = projects.reduce((s,p)=>s+r[p.id],0);
        let m = mainRows.find(x=>x.id===ex.id)
              || mainRows.find(x=>(x.name||"").trim().toLowerCase()===ex.label.trim().toLowerCase());
        if(m){
          const qv = toNum(m.monthly?.T1)+toNum(m.monthly?.T2)+toNum(m.monthly?.T3);
    ...
    const otherCols = [
      ...
      {
        field:"carryOver", headerName:"Vượt lũy kế đầu kỳ", width:180, type:"number", align:"right", headerAlign:"right",
        editable:params=>!isFixed(params.id),
        renderEditCell:p=>(
          <TextField autoFocus fullWidth value={p.value}
    ...
    const isCellEditable = params=> !isFixed(params.id)&&["label","pct","allocated","carryOver"].includes(params.field);
    ...
    const processRowUpdate = newRow=>{
      ...
      setExtraRows(rs=>rs.map(x=>x.id===newRow.id
    ...
    const save=async()=>{
      setSaving(true);
      try{
        await setDoc(doc(db,COL_QUARTER,`${year}_${quarter}`),
      ...
      <Box sx={{bgcolor:theme.palette.background.default,minHeight:"100vh",px:1}}>
        ...
        <Snackbar open={snack.open} autoHideDuration={5000} onClose={handleCloseSnack}>
          <Alert onClose={handleCloseSnack} severity="success" sx={{width:"100%"}}
                 action={snack.action&&<Button color="inherit" size="small" onClick={handleCloseSnack}>{snack.action}</Button>}>
            {snack.msg}
          </Alert>
        </Snackbar>
      </Box>
