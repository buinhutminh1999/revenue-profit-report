...
import { Box, Typography, Paper, FormControl, InputLabel, Select, MenuItem, Stack, Alert, Grid, Skeleton, Chip, Divider, Button, Tooltip } from "@mui/material";
import { alpha, styled, useTheme } from "@mui/material/styles";
import { DataGrid, GridToolbarContainer, GridToolbarQuickFilter, GridActionsCellItem } from "@mui/x-data-grid";
import { ArchiveOutlined, TrendingUp, TrendingDown, AttachMoney, ErrorOutline, Add as AddIcon, Delete as DeleteIcon, FileCopy as FileCopyIcon } from "@mui/icons-material";
import { NumericFormat } from "react-number-format";
import { db } from "../services/firebase-config";
import { collection, doc, setDoc, onSnapshot, query, addDoc, deleteDoc, writeBatch, where, getDocs } from "firebase/firestore"; // Thêm where, getDocs
import { toNum } from "../utils/numberUtils";

const categories = [ { id: 'thi_cong', label: 'I. Thi công' }, { id: 'nha_may', label: 'II. Nhà máy' }, { id: 'kh_dt', label: 'III. KH-ĐT' }, { id: 'khac', label: 'IV. Nợ phải thu khác' }];
const StyledDataGrid = styled(DataGrid)(({ theme }) => ({ border: 0, "& .MuiDataGrid-columnHeaders": { backgroundColor: theme.palette.mode === 'light' ? theme.palette.grey[100] : theme.palette.grey[800], borderBottom: `1px solid ${theme.palette.divider}`, color: theme.palette.text.secondary, textTransform: 'uppercase', fontSize: '0.75rem', fontWeight: 600 }, "& .MuiDataGrid-cell": { borderBottom: `1px solid ${theme.palette.divider}`, display: 'flex', alignItems: 'center' }, "& .MuiDataGrid-cell--editing": { display: 'flex', alignItems: 'center', backgroundColor: alpha(theme.palette.secondary.light, 0.1) }, "& .MuiDataGrid-cell--editing.MuiDataGrid-cell--textRight .MuiInputBase-input": { textAlign: 'right' }, "& .MuiDataGrid-iconSeparator": { display: 'none' }, "& .group-header-row": { backgroundColor: alpha(theme.palette.primary.main, 0.08), fontWeight: 'bold', fontSize: '1rem', color: theme.palette.primary.dark }, "& .group-summary-row": { backgroundColor: theme.palette.grey[50], fontWeight: 'bold', "& .MuiDataGrid-cell": { borderTop: `1px solid ${theme.palette.divider}` } }, "& .grand-total-row": { backgroundColor: alpha(theme.palette.primary.main, 0.15), fontWeight: 'bold', fontSize: '0.9rem', color: theme.palette.primary.dark, "& .MuiDataGrid-cell": { borderTop: `2px solid ${theme.palette.primary.main}` } } }));
...
const MetricCard = ({ title, value, icon, color, loading }) => ( <Box sx={{ display: 'flex', alignItems: 'center' }}> <Box sx={{ mr: 1.5, color: `${color}.main` }}>{icon}</Box> <Box> <Typography variant="caption" color="text.secondary" sx={{ display: 'block', lineHeight: 1.2 }}>{title}</Typography> {loading ? <Skeleton width={80} /> : <Typography variant="subtitle1" fontWeight="600"><NumericFormat value={toNum(value)} displayType="text" thousandSeparator="," /></Typography>} </Box> </Box> );
const NoRowsOverlay = () => ( <Stack height="100%" alignItems="center" justifyContent="center" sx={{ color: "text.secondary" }}> <Typography variant="body2"> Không có dữ liệu. </Typography> </Stack> );
...
export default function AccountsReceivable() {
...
}, [selectedYear, selectedQuarter]); // Thêm dependencies
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            ...
            setRows(fetchedRows);
            setIsLoading(false);
        }, (error) => { console.error("Lỗi khi tải dữ liệu:", error); setIsError(true); setIsLoading(false); });
        return () => unsubscribe();
    }, [selectedYear, selectedQuarter]);

   // Thay thế toàn bộ đoạn code này vào file của bạn
...
const mainColumns = useMemo(() => [
    { 
        ...
        flex: 1, 
        editable: true, 
        renderCell: (params) => (params.row.type !== 'data') ? <Typography variant="body2" sx={{ fontWeight: 'bold' }}>{params.value}</Typography> : params.value, 
    },
    { 
        field: "openingDebit", 
        headerName: "Phải Thu ĐK", 
        type: "number", 
        width: 150, 
        align: "right", 
        headerAlign: "right", 
        editable: true, 
        renderCell: (params) => {
            // Sửa ở đây: Nếu không phải dòng dữ liệu thì không hiển thị gì cả
            if (params.row.type === 'header') return ''; 
            return <CurrencyDisplay value={params.value} typographyProps={{ align: 'right' }} />;
        }
    },
    { 
        field: "openingCredit", 
        headerName: "Trả Trước ĐK", 
        type: "number", 
        width: 150, 
        align: "right", 
        headerAlign: "right", 
    ...
    { 
        ...
        renderCell: (params) => {
            if (params.row.type === 'header') return '';
            if (toNum(params.value) > 0 && params.row.type === 'data') {
                return (<Box sx={{ width: '100%', display: 'flex', justifyContent: 'flex-end' }}><Chip label={<NumericFormat value={toNum(params.value)} displayType="text" thousandSeparator="," />} color="warning" variant="light" size="small"/></Box>);
            }
            return <CurrencyDisplay value={params.value} typographyProps={{ align: 'right' }}/>;
        } 
    },
    { 
        ...
        renderCell: (params) => {
            if (params.row.type === 'header') return '';
            if (toNum(params.value) > 0 && params.row.type === 'data') {
                return (<Box sx={{ width: '100%', display: 'flex', justifyContent: 'flex-end' }}><Chip label={<NumericFormat value={toNum(params.value)} displayType="text" thousandSeparator="," />} color="success" variant="light" size="small"/></Box>);
            }
            return <CurrencyDisplay value={params.value} typographyProps={{ align: 'right' }}/>;
        }
    },
    { 
        ...
        renderCell: (params) => {
            if (params.row.type === 'header') return '';
            return <CurrencyDisplay value={params.value} typographyProps={{ fontWeight: "bold", align: 'right' }} />;
        }
    },
    { 
        field: "closingCredit", 
        headerName: "Trả Trước CK", 
        type: "number", 
        width: 150, 
        align: "right", 
        headerAlign: "right", 
...
], [handleDeleteRow, handleAddRow]); // Thêm handleDeleteRow và handleAddRow vào dependencies

    useEffect(() => {
        const handlePaste = async (event) => {
            if (!activeCell || !activeCell.category) { return; }
            
            const categoryLabel = categories.find(c => c.id === activeCell.category)?.label || "nhóm này";
            // --- THÊM XÁC NHẬN TRƯỚC KHI THAY THẾ ---
            const confirmed = window.confirm(
                `Thao tác này sẽ XOÁ TOÀN BỘ dữ liệu hiện có trong nhóm "${categoryLabel}" và thay thế bằng dữ liệu mới từ clipboard.\n\nBạn có chắc chắn muốn tiếp tục không?`
            );
            if (!confirmed) return;
            ...
            try {
                ...
                // --- BƯỚC 1: TÌM VÀ XÓA CÁC DÒNG CŨ TRONG NHÓM ---
                ...
                // --- BƯỚC 2: THÊM CÁC DÒNG MỚI TỪ CLIPBOARD ---
                ...
                await batch.commit();
                alert(`Đã cập nhật thành công ${parsedRows.length} dòng cho nhóm "${categoryLabel}".`);
            } catch (error) {
                console.error("Lỗi khi dán và cập nhật dữ liệu:", error);
    ...
    }, [activeCell, mainColumns, selectedYear, selectedQuarter]);

    const processRowUpdate = async (newRow) => {
        const { type, ...rowData } = newRow;
        const collectionPath = `accountsReceivable/${selectedYear}/quarters/Q${selectedQuarter}/rows`;
        const rowDocRef = doc(db, collectionPath, rowData.id);
        try { await setDoc(rowDocRef, rowData, { merge: true }); setRows(rows.map((row) => (row.id === newRow.id ? newRow : row))); return newRow; } 
        catch (error) { console.error("Lỗi cập nhật dòng:", error); return rows.find(row => row.id === newRow.id); }
    };
    ...
    const displayRows = useMemo(() => {
        ...
        categories.forEach(category => {
            ...
            const summary = categoryRows.reduce((acc, row) => {
                acc.openingDebit += toNum(row.openingDebit); acc.openingCredit += toNum(row.openingCredit); acc.debitIncrease += toNum(row.debitIncrease);
                acc.creditDecrease += toNum(row.creditDecrease); acc.closingDebit += toNum(row.closingDebit); acc.closingCredit += toNum(row.closingCredit);
                return acc;
    ...
    const handleCellClick = (params) => {
        if (params.row.type === 'data') setActiveCell({ id: params.id, field: params.field, category: params.row.category });
    ...
    return (
        <Box sx={{ bgcolor: "background.default", minHeight: "100vh", p: { xs: 2, sm: 3 } }} >
            ...
            <Paper ref={gridContainerRef} elevation={0} sx={{ borderRadius: 4, overflow: "hidden", border: '1px solid', borderColor: 'divider' }}>
                <Box sx={{ width: "100%" }}>
                    {isError ? ( <Alert severity="error" icon={<ErrorOutline />} sx={{ m: 2 }}> Đã có lỗi xảy ra khi tải hoặc lưu dữ liệu. </Alert> ) : (
                        <StyledDataGrid
                            rows={displayRows} columns={mainColumns} getRowId={(row) => row.id} loading={isLoading} editMode="row"
                            processRowUpdate={processRowUpdate} onProcessRowUpdateError={(error) => console.error(error)}
                            ...
                            disableRowSelectionOnClick autoHeight hideFooter
                        />
                    )}
                </Box>
            </Paper>
