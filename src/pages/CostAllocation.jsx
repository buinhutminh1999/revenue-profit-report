  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      async (snap) => {
        ...
        if (!snap.exists()) {
          const prev = getPrevQuarter();
          const psnap = await getDoc(doc(db, "costAllocations", `${prev.year}_${prev.quarter}`));
  ...
  const handleChange = (id, field, val, sub) => {
    ...
    setRows((p) =>
      p.map((r) => {
        ...
        if (field === "name" && fixedNames.includes((val || "").trim().toLowerCase())) {
          n.fixed = true;
        }
        return n;
      })
...
const handleSave = async () => {
  try {
    ...
    const dataToSave = rows.map((r) => {
      ...
      const factoryValue = Math.round((qv * parseValue(r.percentage))    / 100);
    ...
    return rows
      ...
      .reduce(
        (acc, { monthly, percentage, percentThiCong, percentKHDT }) => {
          ...
          acc.thiCong += Math.round((qv * parseValue(percentThiCong)) / 100);
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
          ...
          />
          <Select value={quarter} sx={{ width: 120 }} onChange={(e) => setQuarter(e.target.value)}>
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
          <Button variant="outlined" startIcon={<ContentCopy />} onClick={handleCopyPrev}>
            Sao chép quý trước
          </Button>
        ...
        >
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center" sx={{ bgcolor: "#1976d2", color: "#fff", px: 1 }}>
                {quarterLabel}
              </TableCell>
              {[
                ...
                <TableCell
                  ...
                  sx={{
                    ...
                    color: "#fff",
                    px: 1,
                    display: hideXs && isXs ? "none" : "table-cell",
                  }}
                >
                  {label}
                </TableCell>
                ...
                <React.Fragment key={r.id}>
                  <TableRow>
                    ...
                    {isRent ? (
                      <TableCell
                        colSpan={isSm ? 1 : 3}
                        sx={{ px: 1, display: isSm ? "none" : "table-cell" }}
                      >
                    ...
                    <TableCell align="center" sx={{ px: 1, display: isXs ? "none" : "table-cell" }}>
                      <EditableCell
                        value={r.percentage}
                        isNumeric
                        type="number"
                    ...
                    <TableCell align="center" sx={{ px: 1 }}>
                      {!r.fixed && lc !== "lương tăng ca" && (
                        <Button size="small" color="error" onClick={() => handleDel(r.id)}>
                  ...
                  {idx === fixedRows.length - 1 && (
                    <TableRow sx={{ background: "#f5f5f5", borderTop: "2px solid #1976d2" }}>
                      <TableCell
                        sx={{
                          position: "sticky",
                          left: 0,
                      ...
                      {["T1", "T2", "T3"].map((sf) => (
                        <TableCell
                          key={sf}
                          align="center"
                          sx={{ px: 1, fontWeight: "bold", display: isSm ? "none" : "table-cell" }}
                        >
