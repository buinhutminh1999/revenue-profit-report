...
const sumQuarter = (m) => ["T1","T2","T3"].reduce((s,k)=>s+parseValue(m[k]||0),0);
...
function EditableCell({ value, onChange, type="text", isNumeric=false }) {
  ...
  return edit ? (
    ...
    <Box
      ...
      sx={{ width:"100%", minHeight:40, display:"flex", alignItems:"center", justifyContent:"center",
...
export default function CostAllocation() {
    ...
    const unsub = onSnapshot(
      ...
      snap=>{
        const list = snap.docs.map(d=>{
          const data = d.data();
          return data.label ?? data.name ?? d.id;
        });
        setCategories(list.sort());
      },
      err=>showSnack(`Không đọc danh sách khoản mục: ${err.message}`,"error")
    );
    return ()=>unsub();
  },[showSnack]);
  ...
  const handleSave = async () => {
    try{
      ...
      await setDoc(doc(db,"costAllocations",docId),{
        rows:toSave,
      ...
      <Box sx={{ mb:3, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <Box sx={{ display:"flex", alignItems:"center", gap:2 }}>
          <TextField
            ...
            sx={{ width:100 }}
          />
          <Select value={quarter} onChange={(e)=>setQuarter(e.target.value)} sx={{ width:120 }}>
      ...
      <TableContainer component={Paper} sx={{ boxShadow:3 }}>
        <Table size="small">
          <TableHead sx={{ bgcolor:"#1976d2" }}>
            <TableRow>
              <TableCell sx={{ color:"#fff", width:220 }}>Khoản mục</TableCell>
              {monthLabels.map(m=>(
              ...
              return (
                <TableRow key={rw.id}>
                  ...
                  <TableCell align="center">
                    {rw.fixed || isRent ? qv.toLocaleString() : (
                      <EditableCell
